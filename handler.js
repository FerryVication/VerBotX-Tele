...
export async function handler(m) {
  ...
  try {
    ...
    if (fromUser?.username && fromUser?.id) {
      global.usernameCache[String(fromUser.username).toLowerCase()] =
        fromUser.id;
    }
    const repliedFrom = baseMsg?.reply_to_message?.from;
    if (repliedFrom?.username && repliedFrom?.id) {
      global.usernameCache[String(repliedFrom.username).toLowerCase()] =
        repliedFrom.id;
    }
    ...
    if (user) {
      ...
      if (!("lastReset" in user))
        user.lastReset = moment().tz("Asia/Jakarta").format("YYYY-MM-DD");
    } else {
          ...
          try {
            if (!isPrems && !isOwner) {
              ...
              if (user.limit < requiredLimit) {
                if (user.limit === 0) await m.reply("Limit kamu habis", m);
                  ...
                  await m.reply(
                    `Limit tidak mencukupi, membutuhkan ${requiredLimit}, dan kamu hanya mempunyai ${user.limit}`,
                    m,
                  );
                continue;
              }
            ...
            if (!isPrems && !isOwner && m._actionSuccess === true) {
              try {
                ...
                console.error(
                  "Gagal mengurangi/menginformasikan limit (callback flow):",
                  e,
                );
              }
            }
      ...
      if (match && usedPrefix && m.text) {
        ...
        if (!isPrems && !isOwner && pluginData.limit) {
          ...
          const isYtmp4Cmd = (() => {
            try {
              ...
              if (cmd instanceof RegExp)
                return /yt(mp4|v|video)/i.test(String(cmd));
              if (Array.isArray(cmd))
                return cmd.some((c) =>
                  c instanceof RegExp
                    ? /yt(mp4|v|video)/i.test(String(c))
                    : /^(ytmp4|ytv|ytvideo)$/i.test(String(c)),
                );
              if (typeof cmd === "string")
                return /^(ytmp4|ytv|ytvideo)$/i.test(cmd);
            } catch {}
            return false;
          })();
          ...
          if (isYtmp4Cmd) {
            ...
            if (
              !global.db.data.users[m.sender] ||
              typeof global.db.data.users[m.sender].limit !== "number"
            ) {
              global.db.data.users[m.sender] =
                global.db.data.users[m.sender] || {};
            ...
            if (have < requiredLimit) {
              if (have === 0) {
                ...
                await m.reply(
                  `Limit tidak mencukupi, membutuhkan ${requiredLimit}, dan kamu hanya mempunyai ${have}`,
                  m,
                );
              }
              continue;
          ...
          if (!isPrems && !isOwner && pluginData.limit) {
            if (typeof m.limit === "number" && m.limit > 0) {
              ...
              try {
                ...
                console.error(
                  "Gagal mengurangi/menginformasikan limit (after success):",
                  e,
                );
              }
            }
...
export async function participantsUpdate(ctx) {
  try {
    ...
    if (ctx.myChatMember) {
      ...
      userName =
        ctx.myChatMember.new_chat_member.user.first_name ||
      ...
      if (
        status === "member" &&
        ctx.myChatMember.old_chat_member.status === "left"
      ) {
        eventType = "join";
      } else if (
        status === "left" &&
    ...
    try {
      if (!global.usernameCache) global.usernameCache = {};
      const userObj =
        ctx.myChatMember?.new_chat_member?.user ||
        ctx.message?.left_chat_member ||
        (ctx.message?.new_chat_members ? null : null);
      if (userObj?.username && userObj?.id) {
        global.usernameCache[String(userObj.username).toLowerCase()] =
          userObj.id;
      }
      if (ctx.message && ctx.message.new_chat_members) {
        for (const member of ctx.message.new_chat_members) {
          if (member?.username && member?.id) {
            global.usernameCache[String(member.username).toLowerCase()] =
              member.id;
          }
        }
      }
...
global.dfail = async (type, m, conn) => {
  try {
    ...
    const text =
      typeof type === "string" && map[type]
        ? map[type]
        : map.error || "An error occurred, please try again later.";

    if (m?.reply) return await m.reply(text);
    if (conn?.sendMessage && m?.chat)
