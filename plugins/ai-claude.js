/*
* CLAUDE AI From api.ferdev.my.id
* Code by FeriPratama
* Bukan eai 
*/
...
const handler = async(m, {conn, usedPrefix, command, text}) => {
  ...
  try {
    await m.reply(wait);
    const request = await fetch(`${APIs.ferdev}/ai/claude?prompt=${encodeURIComponent(text)}&apikey=${ferr}`);
...
handler.help = ['claude'];
handler.tags = ['ai'];
handler.command = /^(claude|claude3(ai)?)$/i;
handler.limit = true
