/*
* AICODING From api.ferdev.my.id
* Code by FeriPratama
* Bukan eai 
*/
...
const handler = async(m, {conn, usedPrefix, command, text}) => {
  ...
  try {
    await m.reply(wait);
    const request = await fetch(`${APIs.ferdev}/ai/aicoding?prompt=${encodeURIComponent(text)}&apikey=${ferr}`);
...
handler.help = ['aicoding'];
handler.tags = ['ai'];
handler.command = /^(aicoding|coding(ai)?)$/i;
handler.limit = true
