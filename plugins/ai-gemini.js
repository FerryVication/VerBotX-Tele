/*
* GEMINI AI From api.ferdev.my.id
* Code by FeriPratama
* Bukan eai 
*/
...
const handler = async(m, {conn, usedPrefix, command, text}) => {
  ...
  try {
    const request = await fetch(`${APIs.ferdev}/ai/gemini?prompt=${encodeURIComponent(text)}&apikey=${ferr}`);
...
handler.help = ['gemini'];
handler.tags = ['ai'];
handler.command = /^(gemini|gemini(ai)?)$/i;
handler.limit = true
