...
function start(file) {
  ...
  try {
    ...
    console.error(
      "\x1b[31m%s\x1b[0m",
      `❌ Telegraf library is not installed. Please run: npm install telegraf`,
    );
  }
  ...
  fs.readdir(pluginsFolder, (err, files) => {
    ...
    if (!Array.isArray(files) || files.length === 0) {
      console.log(
        "\x1b[33m%s\x1b[0m",
        `🟡 No plugins found in folder ${pluginsFolder}`,
      );
      return;
    }
    console.log(
  ...
  console.log("\x1b[33m%s\x1b[0m", `📃 Modified by FeriPratama`);
