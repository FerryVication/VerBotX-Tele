...
export default async (m, conn = { logger: console }, isOutgoing = false) => {
  ...
  if (isOutgoing) {
    ...
    if (m.fakeObj?.message?.photo) {
      messageType = "Photo";
      filesize =
        m.fakeObj.message.photo[m.fakeObj.message.photo.length - 1].file_size ||
        0;
  ...
  const botInfo = conn.botInfo || {
    username: global.botname || "telegram_bot",
    first_name: global.botname || "Bot",
  };
...
`.trim(),
    ...
    filesize === 0
      ...
      : (
          filesize /
          1000 ** Math.floor(Math.log(filesize) / Math.log(1000))
        ).toFixed(1),
  ...
  if (!isOutgoing) {
    if (m.fakeObj?.message?.document && m.fakeObj.message.document.file_name) {
      ...
      console.log(
        `👨 ${m.fakeObj.message.contact.first_name || ""} ${m.fakeObj.message.contact.last_name || ""}`,
      );
    } else if (m.fakeObj?.message?.audio) {
      ...
      console.log(
        `📍 Location: ${m.fakeObj.message.location.latitude}, ${m.fakeObj.message.location.longitude}`,
      );
    }
  }
