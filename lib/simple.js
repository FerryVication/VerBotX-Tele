...
const downloadFromMessage = async (ctx) => {
  try {
    ...
    const getFile = async () => {
      ...
      if (quoted.document)
        return await ctx.telegram.getFile(quoted.document.file_id);
      if (quoted.sticker)
        return await ctx.telegram.getFile(quoted.sticker.file_id);
      return null;
    };
  ...
  conn.sendMessage = async (jid, content, options = {}) => {
    try {
      ...
      if (photo || image) {
        ...
        print(
          {
            content: { photo: imageInput, caption: content.caption },
            chat: jid,
          },
          conn,
          true,
      ...
      if (video) {
        ...
        print(
          { content: { video, caption: content.caption }, chat: jid },
          conn,
          true,
        );
        return result;
      ...
      if (audio) {
        ...
        print(
          { content: { audio, caption: content.caption }, chat: jid },
          conn,
          true,
        );
        return result;
      ...
      if (document) {
        ...
        const result = await conn.telegram.sendDocument(
          jid,
          processedDocument,
          opts,
        );
        print(
          { content: { document, caption: content.caption }, chat: jid },
          conn,
          true,
        );
        return result;
      ...
      if (sticker) {
        ...
        const result = await conn.telegram.sendSticker(
          jid,
          processedSticker,
          opts,
        );
  ...
  conn.sendFile = async (
    ...
    try {
      ...
      if (fileInput.length > MAX_SIZE) {
        ...
        return await conn.telegram.sendDocument(
          jid,
          {
            source: compressedBuffer,
            filename: filename || "file.zip",
      ...
      switch (fileType) {
        ...
        case "audio":
          result = await conn.telegram.sendAudio(
            jid,
            {
              source: fileInput,
              filename: filename,
        ...
        case "sticker":
          result = await conn.telegram.sendSticker(
            jid,
            {
              source: fileInput,
            },
      ...
      print(
        { content: { file: path, type: fileType, caption }, chat: jid },
        conn,
        true,
      );
      return result;
